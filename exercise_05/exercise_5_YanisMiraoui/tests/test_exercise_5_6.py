...
@pytest.mark.parametrize("method", [("append"), ("appendleft"), ("pop"), ("popleft")])
...
def test_popleft():
    ...
    Q.append(4)
    Q.append(6)
    assert Q.popleft() == 2, "expected value of 2"


@pytest.mark.parametrize(
    "items, length", [([1, 2, "a", "abc"], 4), ([2], 1), ([[1, 2, 3], 1, 2], 3)]
)
...
@pytest.mark.parametrize(
    "idx, iterated", [(0, [15, 20, 25, 35, 40]), (1, ["n+3", "n+2", "n", "n+4"])]
)
...
def test_mem_leakage():
    ...
    Q.pop()
    ref.append(sys.getrefcount(A))
    assert np.array_equal(ref, [2, 3, 2]), "expected pop to overwrite place in memory"
