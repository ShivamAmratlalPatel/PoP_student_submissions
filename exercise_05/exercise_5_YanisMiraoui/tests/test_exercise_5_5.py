...
def test_length_implemented():
    assert hasattr(RPCalc(), "__len__"), "RPCalc has no len special method"
...
def test_pop():
    ...
    calc.push(1)
    calc.pop()
    assert not bool(calc), "expected empty stack"
...
def test_peek():
    ...
    calc.push(1)
    calc.peek()
    assert bool(calc), "expected non-empty stack"
...
def test_calculator_arithmetic(expression, answer):
    ...
    for element in expression:
        calc.push(element)
    assert calc.peek() == answer, f"expected an answer of {answer}"
