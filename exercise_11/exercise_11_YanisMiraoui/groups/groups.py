...
class Element:
    ...
    def __repr__(self):
        ...
        return f"{type(self).__name__}" f"({repr(self.group), repr(self.value)})"
...
class CyclicGroup(Group):
    ...
    def _validate(self, value):
        ...
        if not (isinstance(value, Integral) and 0 <= value < self.n):
            raise ValueError(
                "Element value must be an integer" f" in the range [0, {self.n})"
            )
...
class GeneralLinearGroup(Group):
    ...
    def _validate(self, value):
        ...
        if not (value.shape == (self.n, self.n)):
            raise ValueError(
                "Element value must be a " f"{self.n} x {self.n}" "square array."
            )
